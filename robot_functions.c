...
int pureRecursive(int **px, int rows, int cols, int i, int j) {
    if ((i == 0) && (j == 0)) return px[0][0];
    if ((i == 0) && (j < cols))
        return pureRecursive(px, rows, cols, i, j - 1) + px[i][j];
    if ((j == 0) && (i < rows))
        return pureRecursive(px, rows, cols, i - 1, j) + px[i][j];
    if ((i < rows) && (j < cols))
        return (pureRecursive(px, rows, cols, i, j - 1) >
                        pureRecursive(px, rows, cols, i - 1, j)
                    ? pureRecursive(px, rows, cols, i, j - 1)
                    : pureRecursive(px, rows, cols, i - 1, j)) +
               px[i][j];
}
...
int matrRecursive(int **px, int **matrix, int rows, int cols, int i, int j) {
    ...
    if ((i == 0) && (j < cols)) {
        matrix[i][j] =
            matrRecursive(px, matrix, rows, cols, i, j - 1) + px[i][j];
        return matrix[i][j];
    }
    if ((j == 0) && (i < rows)) {
        matrix[i][j] =
            matrRecursive(px, matrix, rows, cols, i - 1, j) + px[i][j];
        return matrix[i][j];
    }
    if ((i < rows) && (j < cols)) {
        matrix[i][j] = (matrRecursive(px, matrix, rows, cols, i, j - 1) >
                                matrRecursive(px, matrix, rows, cols, i - 1, j)
                            ? matrRecursive(px, matrix, rows, cols, i, j - 1)
                            : matrRecursive(px, matrix, rows, cols, i - 1, j)) +
                       px[i][j];
        return matrix[i][j];
    }
