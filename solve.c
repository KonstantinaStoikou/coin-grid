...
int main(void){
	int nrows,mcols,i,j,**grid;
	char x;
	scanf("%d %d", &nrows, &mcols);         //read size of array
	grid = malloc(nrows * sizeof(int *));   //dynamic memory allocation for grid
	if (grid == NULL){ 
		return -1;
	}
	for (i = 0; i < nrows; i++){
	...
	for (i = 0; i < nrows; i++)             //memory deallocation for grid 
			free(*(grid+i));
		free(grid);
	return 0;
}
